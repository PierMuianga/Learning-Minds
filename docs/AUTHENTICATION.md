...
Create a Supabase project. In **Project Settings → API**, copy the Project URL to `NEXT_PUBLIC_SUPABASE_URL` and the publishable key to `NEXT_PUBLIC_SUPABASE_PUBLISHABLE_KEY`. These values are designed to be public; RLS protects data. Never add the service-role key.

Set **Authentication → URL Configuration → Site URL** to the deployed production origin. Add exact redirect URLs for `http://localhost:3000/auth/callback`, the production `https://YOUR-DOMAIN/auth/callback`, and any Vercel preview callback pattern you deliberately support. Add the corresponding origin to `NEXT_PUBLIC_SITE_URL` per environment.
...
In **Vercel → Project → Settings → Environment Variables**, add `NEXT_PUBLIC_SUPABASE_URL`, `NEXT_PUBLIC_SUPABASE_PUBLISHABLE_KEY`, and `NEXT_PUBLIC_SITE_URL` for Production, Preview and Development as appropriate. Redeploy after changing values.
